import type { DurabilityReport, HorizonResult } from "./types.js";

function percent(value: number | undefined): string {
  return value === undefined ? "—" : `${(value * 100).toFixed(1)}%`;
}

function horizonRow(horizon: HorizonResult): string {
...
export function renderPortfolioMarkdown(reports: DurabilityReport[]): string {
  if (reports.length === 0) {
    return [
